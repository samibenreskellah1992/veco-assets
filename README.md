...
Endpoints disponibles à ce stade (Phases 1-7) :
...
Toute autre route est protégée par défaut (`anyRequest().authenticated()`) : un JWT valide (`Authorization: Bearer <token>`) est requis. Les endpoints métier restants (`/api/movements`, `/api/reports`, ...) sont ajoutés phase par phase, voir `docs/ROADMAP.md`.
