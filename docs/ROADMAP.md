...
## 13. Limite connue de l'environnement de développement utilisé pour les Phases 1-4
L'environnement cloud dans lequel les Phases 1 à 4 ont été développées n'a pas accès à `repo.maven.apache.org` (bloqué par sa politique réseau) et n'a pas de démon Docker accessible. Conséquences :
- Le backend Java/Spring Boot n'a **jamais été compilé par Maven** dans cet environnement, phase après phase — le code a été relu attentivement à chaque phase (Phase 3 : signature JWT, wiring Spring Security, méthode par méthode ; Phase 4 : les 7 services CRUD référentiel/utilisateurs et leurs contrôleurs, méthode par méthode, plus un contrôle croisé systématique entre chaque nom de méthode de repository déclaré et chaque site d'appel dans les services) mais `mvn compile` reste à exécuter dans un environnement avec accès réseau normal (poste de développement, CI Vecopharm) avant de considérer une phase backend entièrement close (voir section 11, point 2 "Compilation OK").
- Les migrations Flyway (Phases 2 et 4) ont pu être vérifiées en conditions réelles (rejeu à froid contre un PostgreSQL 16 local, contraintes testées avec des insertions/suppressions invalides) car `psql` était disponible directement, indépendamment de Maven.
- Les tests d'intégration (Phase 3 : `AuthenticationIntegrationTest`, basés sur Testcontainers pour utiliser un vrai PostgreSQL plutôt que H2 — voir Phase 10) n'ont en revanche **pas pu être exécutés du tout** ici : ils nécessitent à la fois Maven et un démon Docker, tous deux absents. Ils ont été écrits et relus avec la même rigueur, mais leur exécution réelle (`mvn test`) reste à faire dans un environnement normal. La Phase 4 n'ajoute pas de nouveaux tests d'intégration backend (aucun module de test supplémentaire n'a pu être exécuté ici de toute façon) ; les 7 nouveaux services sont donc dans la même situation que le reste du backend — relus, non compilés.
- Le frontend, lui, **a pu être vérifié pour de vrai en Phase 4** : `npm run build` (`tsc -b && vite build`) et `npm run lint` (`oxlint`) passent tous les deux sans erreur sur l'ensemble du code Phase 4 (auth, primitives shadcn/ui ajoutées à la main, pages Référentiel et Utilisateurs), le registre npm n'étant pas soumis à la même restriction que Maven Central. Ça ne prouve pas que les appels API fonctionnent contre un vrai backend démarré (non testé de bout en bout ici), seulement que le code TypeScript est correct et que le build de production réussit.

Priorité pour la suite avant la Phase 5 : lancer `mvn compile`/`mvn test` (avec Docker) dans un environnement normal pour confirmer les Phases 2 à 4 côté backend, puis démarrer l'application complète (backend + frontend + PostgreSQL réels) pour vérifier de bout en bout le parcours connexion → Référentiel → Utilisateurs.
